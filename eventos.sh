...
curl -m 20 -s --socks5-hostname localhost:9050 --retry 5 --retry-delay 5 https://proxy.zeronet.dev/18D6dPcsjLrjg2hhnYqKzNh2W6QtXrDwF/links.json > $file1
...
do
	...
	if [[ $(grep -i "teledeporte" $file1) ]]
        then
	       ...
	       echo "https://www.rtve.es/play/videos/directo/deportes/tdp/" >> $canales
	       canal_sin_enlace=false
	fi
